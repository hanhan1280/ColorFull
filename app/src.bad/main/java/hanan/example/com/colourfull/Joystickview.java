...
public class Joystickview extends View  implements   Runnable {
    ...
    */
    ...
    private static final int My_Color_Hat = 0x644b4b4b;
    /**
    ...
    private boolean bDirectionAlreadySent = false;
    ...
    public Joystickview(Context context, AttributeSet attrs) {
        ...
        try {
            buttonColor = styledAttributes.getColor(R.styleable.JoystickView_JV_buttonColor, My_Color_Hat);
            ...
            mButtonSizeRatio = styledAttributes.getFraction(R.styleable.JoystickView_JV_buttonSizeRatio, 1, 1, 0.315f);
            mBackgroundSizeRatio = styledAttributes.getFraction(R.styleable.JoystickView_JV_backgroundSizeRatio, 1, 1, 0.63f);
  ...
  //      canvas.drawCircle(mFixedCenterX, mFixedCenterY, mBorderRadius, drawPaint);
        //canvas.draw

        drawTriangle(R.color.myBLUE, 1 , canvas);
        drawTriangle(R.color.myRED, 2, canvas);
        drawTriangle(R.color.myYELLOW, 3, canvas);
        drawTriangle(R.color.myGREEN,4, canvas);
...
private void drawTriangle(int color , int direction, Canvas canvas){
    int realposX = mFixedCenterX + mBorderRadius;
    int realposY = mFixedCenterY + mBorderRadius;
    int altposX = mFixedCenterX - mBorderRadius;
    int altposY = mFixedCenterY - mBorderRadius;
    Paint drawPaint = new Paint();
    drawPaint.setColor(getResources().getColor(color));
    ...
    if (direction == 1){
        ...
        path.moveTo(mFixedCenterX, altposY - 60);
        path.lineTo(mFixedCenterX - 35, altposY - 15);
        path.lineTo(mFixedCenterX + 35, altposY - 15);
        path.close();
    }
    else if (direction == 2){
        path.moveTo(realposX + 60, mFixedCenterY);
        path.lineTo(realposX + 15, mFixedCenterY + 35);
        path.lineTo(realposX + 15, mFixedCenterY - 35);
        path.close();
    }
    else if (direction == 3){
        path.moveTo(mFixedCenterX, realposY + 60);
        path.lineTo(mFixedCenterX + 35, realposY + 15);
        path.lineTo(mFixedCenterX - 35, realposY + 15);
        path.close();
    }
    else if (direction == 4){
        path.moveTo(altposX - 60, mFixedCenterY);
        path.lineTo(altposX - 15, mFixedCenterY + 35);
        path.lineTo(altposX - 15, mFixedCenterY - 35);
        path.close();
    }
 ...
 //                   mCallback.onMove(getAngle(), getStrength());
                    mCallback.onDirection(DirectionofButton(mPosX , mPosY));
            }
     ...
     //           mCallback.onMove(getAngle(), getStrength());
        ...
        int newPosX = mPosX - mFixedCenterX;
        int newPosY = mPosY - mFixedCenterY;

        //direction vertical
        if (abs(newPosY) > abs(newPosX)){
            mPosX = mFixedCenterX;
        }
        //direction horizontal
        else if (abs(newPosX) > abs(newPosY)){
            mPosY = mFixedCenterY;
        }
        //diagonals
        else if (newPosY == abs(newPosX)){
            mPosX = mFixedCenterX;
        }
        else if (newPosX == abs(newPosY)){
            mPosY = mFixedCenterY;
        }
...
//                mCallback.onMove(getAngle(), getStrength());
    ...
    private int DirectionofButton(int mPosX, int mPosY){
        int newPosX = mPosX - mFixedCenterX;
        int newPosY = mPosY - mFixedCenterY;
        int negPosX = mFixedCenterX - mPosX;
        int negPosY = mFixedCenterY - mPosY;
        int mdirection = 0;
        //Direction Up
        if(negPosY >= mBorderRadius - 15){
            mdirection = 1;
        }
        //Direction Right
        else if(newPosX >= mBorderRadius - 15){
            mdirection = 2;
        }
        //Direction Left
        else if(negPosX >= mBorderRadius - 15){
            mdirection = 4;
        }
        else if(newPosY >= mBorderRadius - 15){
            mdirection = 3;
        }
        return mdirection;
    }
    ...
    public void run() {
        while (!Thread.interrupted()) {
                ...
                newdirection = DirectionofButton(mPosX, mPosY);

            post(new Runnable() {
                public void run() {
                    if (mCallback != null) {
                            //mCallback.onMove(getAngle(), getStrength());
                            mCallback.onDirection(newdirection);
